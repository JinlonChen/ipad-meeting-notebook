...
import { ALIBABA_REALTIME_BASE_URL, ALIBABA_REALTIME_MODEL, type AiProviderConfiguration } from "./api.js";
...
export function AiSettingsPage({ api }: { api: AiSettingsPort }) {
  ...
  return <main className="workspace-shell ai-settings-page"><header className="workspace-topbar"><button className="icon-button" aria-label="返回会议" title="返回会议" onClick={() => navigate("/meetings")}><ChevronLeft size={18} /></button><h1>AI 设置</h1></header>
    <form className="ai-settings-form" onSubmit={(event) => void submit(event)}>
      <fieldset><legend>语音转写</legend><label>转写接口地址<input name="transcriptionBaseUrl" type="url" inputMode="url" value={ALIBABA_REALTIME_BASE_URL} readOnly required /></label><label>转写模型<input name="transcriptionModel" value={ALIBABA_REALTIME_MODEL} readOnly required /></label><label>转写 API Key<input name="transcriptionApiKey" type="password" autoComplete="off" required /></label></fieldset>
