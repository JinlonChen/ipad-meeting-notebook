...
export function MeetingWorkspacePage({ repository, refresh, scheduleRefresh = refresh, online, recorder, now = () => new Date().toISOString(), intelligence }: Props) {
  const { id = "" } = useParams();
  ...
  return <MeetingEditor meeting={loadState.meeting} folders={loadState.folders} noteSyncState={loadState.noteSyncState} repository={repository} recorder={recorder} scheduleRefresh={scheduleRefresh} online={online} now={now} {...(intelligence ? { intelligence } : {})} />;
}
...
function MeetingEditor({ meeting, folders, noteSyncState, repository, recorder, scheduleRefresh, online, now, intelligence }: EditorProps) {
  const navigate = useNavigate();
