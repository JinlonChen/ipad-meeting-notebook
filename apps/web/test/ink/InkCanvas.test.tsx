...
test("clamps a deep canvas after width shrink and saves at the visible logical bottom", async () => {
  let width = 512;
  ...
  const save = vi.fn().mockResolvedValue(undefined);
  render(<InkCanvas meetingId={stroke.meetingId} initialStrokes={[deepStroke]} onSave={save} />);
  const canvas = screen.getByLabelText("手写画布") as HTMLCanvasElement;
  expect(canvas.style.height).toBe("50000px");

  width = 256;
  resize?.([], {} as ResizeObserver);
  await waitFor(() => expect(canvas.style.height).toBe("25000px"));

  pointer(canvas, "pointerdown", { pointerId: 14, pointerType: "pen", clientX: 20, clientY: 24_900, pressure: 0.5 });
  pointer(canvas, "pointerup", { pointerId: 14, pointerType: "pen", clientX: 30, clientY: 24_920, pressure: 0.5 });
  await waitFor(() => expect(save).toHaveBeenCalledOnce());
  expect(save.mock.calls[0]?.[0][0].points.every((point: { y: number }) => point.y <= INK_LOGICAL_HEIGHT)).toBe(true);
